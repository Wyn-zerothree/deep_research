...
3. **本节所有检索指标只覆盖本地 RAG 一路 —— 跑批时网络检索是被主动关掉的。** 早期为省调用费用，跑批命令写成 `BOCHA_API_KEY= python run_eval.py`，把该变量显式设成**空字符串**；而 `load_dotenv` 默认 `override=False`、**不覆盖已存在的环境变量，空串也算"存在"**，于是空值生效，`tools.py` 每次都在 `if not api_key: return []` 处退出，**一次真实请求都没发出去**。之所以长期没暴露，是因为记录里的 `web_query_count` 数的是"发起次数"而非"成功次数"，看着一直正常。`run_eval.py` 现已启动时检查该变量、并把 `web_search_enabled` 写进每条记录（`4fa8b7a`）。
   2026-09-25 补齐了带 key 的 3 题样本：96 次 Bocha 请求全部 `status=200`，网络路产出 WEB 来源 25 / 18 / 33 条、去重引文 14 / 17 / 20 条、引用合规率均为 1.000，**但 3 题里只有 2 题可用**（1 题是降级报告）。而且评测集的 ground truth 是**本地语料文件名**，网络路没有对应基准，**算不出召回**。所以准确说法是：**"网络路链路已验证可用、引文可溯源"成立；"双路检索的检索质量指标"不成立。**
4. **重试是整题从头重跑，对按次计费的外部检索是乘法级成本风险。** `run_eval.py` 的重试不保留任何中间产物，一次网络抖动就把这题的全部 LLM 调用和全部 Bocha 查询再买一遍：3 题实测共 96 次 Bocha 调用，其中 **65 次（68%）是重试烧掉的**，总花费 ≈ ¥3.76 而 Bocha 占 92%。且**越晚崩越贵**（退在 `analyze` 节点烧 16 次，退在 `web_search` 节点烧 22 次，崩之前买的检索结果全部丢弃）。重试通过也不代表记录干净 —— 有题目重试 2 次后落盘仍带 `degraded_nodes=1`。改进方向是让重试复用已落盘的 `web_search_trace` 证据，尚未实施。
